...
const MapTable: React.FC<IWinCountsWrap> = ({ winCounts, sortBy, resultLimit}) => {
  const data = React.useMemo(
    ():IMapData[] => {
      return Object.keys(winCounts).map((map) => {
        const won = winCounts[map].wins;
        const played = won + winCounts[map].losses;
        return {
          map: map,
          played: played,
    ...
    ():Column<IMapData>[] => [
      {
        ...
        id: 'map',
        accessor: 'map',
        Cell: ({cell: {value}}) => <div className="map-with-name">
          <MapIcon map={value}/>
          <label>{value}</label>
        </div>
      },
  ...
  const tableInstance = useTable<IMapData>({
    columns, data,
    initialState: {
  ...
  return (
    <table className="map-table" {...getTableProps()}>
      <thead>
